...
* Had a Wrong Answer attempt before the accepted one, likely from returning 0-based indices instead of the required 1-based indices, or from mishandling the loop bound.
* Forgot that the array is already sorted, which makes the two-pointer approach possible instead of a HashMap.
